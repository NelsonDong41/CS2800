...
def lenmul4 : list ℕ -> bool
 | [] := tt
 | (list.cons a (list.cons b (list.cons c (list.cons d L)))) := lenmul4 L
 | (list.cons b (list.cons c (list.cons d L))) := ff
 | (list.cons c (list.cons d L)) := ff
 | (list.cons d L) := ff
...
def app : list ℕ → list ℕ → list ℕ 
 ...
 | n [] := n 
 | [] n := n
 | (a :: L) n := (list.cons a (app L n)) 

def rl: list ℕ -> ℕ -> list ℕ 
 | [] _ := []
 | L 0 := L 
 | (x :: L) (y+1) := rl (app L [x]) y
...
-/
-- ANSWER:
example: apply [1,2,3] (fun x, 2 * x) = [2,4,6] := begin refl, end 
example: apply [1,2,3] (fun x, x * x) = [1,4,9] := begin refl, end 
example: apply [1,2,3] (fun x, if x < 2 then x else 2) = [1,2,2] := begin refl, end 
...
-/
example: apply [1,2,3] (fun x, 0) = [0,0,0] := begin refl, end 
example: apply [21,2,3,3] (fun x, 0) = [0,0,0,0] := begin refl, end 
example: apply [1,2,3,4,5,6,7] (fun x, 0) = [0,0,0,0,0,0,0] := begin refl, end 
...
inductive btn : Type 
 | leaf (x: ℕ): btn
 | node (left: btn) (right: btn) (x : ℕ): btn 
...
-- ANSWER:
#check btn.leaf 6 
#check btn.node  (btn.leaf 5) (btn.leaf 6) (1)
...
def btn2natlist: btn -> list ℕ 
 | (btn.leaf x) := list.cons (x:ℕ) []
 | (btn.node left right n) := list.cons n (app (btn2natlist right) (btn2natlist left))

example: btn2natlist (btn.leaf 10) = [10] := begin refl, end 
example: btn2natlist (btn.node (btn.leaf 2) (btn.leaf 1) 0) = [0,1,2] := begin refl, end 
example: btn2natlist (btn.node (btn.node (btn.leaf 2) (btn.leaf 2) 1) (btn.leaf 0) 0) = [0,0,1,2,2] := begin refl, end 
example: btn2natlist (btn.node (btn.node (btn.leaf 2) (btn.leaf 1) 0) (btn.leaf 1) 0) = [0,1,0,1,2] := begin refl, end 
...
def btnleaves2natlist: btn -> list ℕ 
 | (btn.leaf x) := list.cons (x:ℕ) []
 | (btn.node left right n) := app (btnleaves2natlist right) (btnleaves2natlist left)

example: btnleaves2natlist (btn.leaf 10) = [10] := begin refl, end 
example: btnleaves2natlist (btn.node (btn.leaf 2) (btn.leaf 1) 5) = [1,2] := begin refl, end 
example: btnleaves2natlist (btn.node (btn.node (btn.leaf 2) (btn.leaf 2) 1) (btn.leaf 1) 1) = [1,2,2] := begin refl, end 
example: btnleaves2natlist (btn.node (btn.node (btn.leaf 0) (btn.leaf 0) 1) (btn.node (btn.leaf 0) (btn.leaf 0) 0) 1) = [0,0,0,0] := begin refl, end 
...
def btnnodes2natlist: btn -> list ℕ 
 | (btn.leaf x) := []
 | (btn.node left right n) := list.cons n (app (btnnodes2natlist right) (btnnodes2natlist left))

example: btnnodes2natlist (btn.node (btn.leaf 2) (btn.leaf 1) 10)  = [10] := begin refl, end 
example: btnnodes2natlist (btn.node (btn.node (btn.leaf 0) (btn.leaf 0) 2) (btn.leaf 0) 1) = [1,2] := begin refl, end 
example: btnnodes2natlist (btn.node (btn.node (btn.leaf 2) (btn.leaf 1) 0) (btn.node (btn.leaf 0) (btn.leaf 1) 0) 0) = [0,0,0] := begin refl, end 
example: btnnodes2natlist (btn.node (btn.node (btn.node (btn.leaf 3) (btn.leaf 7) 0) (btn.leaf 1) 0) (btn.node (btn.leaf 0) (btn.leaf 1) 0) 0) = [0,0,0,0] := begin refl, end 
...
def reducenat : mynat -> mynat
  | Z := Z
  | (S Z) := (S Z)
  | (S (S x)) := reducenat x

-- checks nat equality for Z and (S Z) comparisons, otherwise returns false
def mynateq : mynat -> mynat -> bool
  | Z Z := tt 
  | Z (S Z) := ff
  | (S Z) Z := ff
  | (S Z) (S Z) := tt
...
def myodd (x: mynat) : bool := if (mynateq (reducenat x) Z) then ff else tt
...
def mymult : mynat → mynat → mynat 
  ...
  | (S Z) (S x) := (S x)
  | (S x) (S Z) := (S x)
  | (S x) (S y) := (mymult (myplus (S x) (S x)) y)
...
def fB (n1 : ℕ) (z : ℤ) (n2 : ℕ) :  ℤ 
:= n1 + n2 - z

#check nat -> int -> nat -> int 
#check fB 

def fC (n : ℕ ) (f: ℤ -> ℕ ) : bool
:= if (n = (f (1:ℤ))) then tt else ff

#check nat -> (int -> nat) -> bool  
#check fC 

def fD (n : ℕ) (f : ℤ -> ℕ ) (z : ℤ) : bool
:= if (n = (f z)) then tt else ff

#check nat -> (int -> nat) -> int -> bool 
#check fD  

def fE (b: bool) : (ℕ -> (ℤ -> ℕ ) -> ℤ)
:= if b = tt then (fun (x:ℕ) (f: ℤ -> ℕ), ((f 1 + x):ℤ)) else (fun (x:ℕ) (f: ℤ -> ℕ), ((f 2 + x):ℤ))
