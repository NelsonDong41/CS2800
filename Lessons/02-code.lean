...
def nonterminatingbounded : ℕ → ℕ → ℕ   -- the bound is the 2nd argument
    ...
    | (n + 1) 0 := 0  -- when the second argument (the bound) reaches 0, we MUST terminate. we can choose to return anything we want (something that makes sense for the function we are trying to define). here we chose 42 ... 
...
#eval nonterminatingbounded 3 10 
